...
class ArxivDigest():
    ...
    def get_recent_papers(self, category):
        id_list = a.get_most_recent_ids(category)
        id_string = ""
        for item in id_list:
            id_string += f"{item},"
        res = a.query_arxiv(f"cat:{category}&id_list={id_string}", len(id_list))
        res = a.extract_key_fields_from_query_response(res)
        for paper in res:
            self.db[paper['primary_category']["@term"]][paper['title']] = paper
        self.save_db()
        return res
